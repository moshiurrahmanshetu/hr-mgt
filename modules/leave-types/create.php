...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token()) {
        ...
        if (empty($errors['name'])) {
            try {
                $check_stmt = $pdo->prepare("SELECT id FROM leave_types WHERE name = ?");
        ...
        if (empty($errors)) {
            try {
                $stmt = $pdo->prepare("
                    INSERT INTO leave_types (name, max_days_per_year, status) 
                    VALUES (?, ?, ?)
                ");
                $stmt->execute([$name, intval($max_days), $status]);
                
                log_activity($_SESSION['user_id'], 'leave_type_create', "Created leave type: $name");
                
                redirect_with_flash(BASE_URL . '/modules/leave-types/list.php', 'success', 'Leave type created successfully!');
                
            } catch (PDOException $e) {
                error_log("Leave type creation error: " . $e->getMessage());
                $errors[] = 'An error occurred while creating the leave type.';
            }
        }
    }
}
...
<div class="row mb-4">
    <div class="col">
        <h2 class="fw-bold">Create Leave Type</h2>
        <p class="text-muted">Add a new leave type category</p>
    </div>
</div>
