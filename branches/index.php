<?php
/**
 * Branches Directory (Organization Module)
...
if (!empty($search)) {
    $where_clauses[] = "(b.name LIKE :search OR b.code LIKE :search OR b.address LIKE :search OR b.phone LIKE :search OR b.email LIKE :search)";
...
$count_sql = "SELECT COUNT(*) FROM `branches` b WHERE $where_sql";
$count_stmt = $db->prepare($count_sql);
$count_stmt->execute($params);
$total_rows = (int)$count_stmt->fetchColumn();
$total_pages = ceil($total_rows / $limit);

// Fetch Query (Joining employees to get Manager's Name)
$fetch_sql = "SELECT b.*, e.first_name AS manager_first, e.last_name AS manager_last, e.employee_code AS manager_code
              FROM `branches` b
...
$branches_list = $fetch_stmt->fetchAll();
...
<div class="main-content">
    <div class="content-body" data-aos="fade-up">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
            <div>
                <h2 class="fw-bold tracking-tight mb-1" style="color: var(--text-primary);">Branches</h2>
                <p class="text-muted small mb-0">Establish, organize, and manage corporate physical office branches and operational sites.</p>
            </div>
            <div>
                <a href="<?php echo base_url('index.php?route=branches-create'); ?>" class="btn btn-primary btn-sm d-inline-flex align-items-center gap-1.5 px-3 py-2 fw-semibold">
            ...
            <form action="<?php echo base_url('index.php'); ?>" method="GET" class="row g-2.5">
                ...
                <div class="col-12 col-md-5">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="search" class="form-control border-start-0" placeholder="Search by name, code, phone, address..." value="<?php echo sanitize($search); ?>" style="background-color: var(--bg-primary); border-color: var(--border-color); color: var(--text-primary);">
                    </div>
                </div>
                ...
                <div class="col-12 col-sm-6 col-md-4 d-flex gap-2">
                    ...
                    <?php if (!empty($search) || !empty($status_filter)): ?>
                        <a href="<?php echo base_url('index.php?route=branches'); ?>" class="btn btn-outline-secondary btn-sm px-3 d-inline-flex align-items-center justify-content-center">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="color: var(--text-primary);">
                    <thead class="table-dark" style="--bs-table-bg: var(--bg-tertiary); --bs-table-border-color: var(--border-color); color: var(--text-secondary);">
                        <tr>
                            <th class="ps-4 py-3" style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Branch Code</th>
                            <th class="py-3" style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Name</th>
                            <th class="py-3" style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Phone / Email</th>
                            <th class="py-3" style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Manager</th>
                        ...
                        <?php if (empty($branches_list)): ?>
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    ...
                                    <h5 class="fw-semibold text-secondary">No Branches Found</h5>
                                    <p class="text-muted small mb-0">No corporate branches match your search and filtering criteria.</p>
                                </td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr style="border-bottom: 1px solid var(--border-color);">
                                    <td class="ps-4">
                                        <span class="font-mono text-xs fw-semibold px-2 py-1.5 rounded" style="background-color: var(--bg-primary); color: var(--accent-primary); border: 1px solid var(--border-color);">
                                            <?php echo sanitize($branch['code']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="fw-semibold" style="font-size: 0.9rem;"><?php echo sanitize($branch['name']); ?></div>
                                        <div class="text-muted small" style="max-width: 250px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">
                                            <i class="bi bi-geo-alt me-1"></i><?php echo $branch['address'] ? sanitize($branch['address']) : 'No address defined'; ?>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="small fw-medium"><?php echo $branch['phone'] ? sanitize($branch['phone']) : 'N/A'; ?></div>
                                        <div class="text-muted small" style="font-size: 0.75rem;"><?php echo $branch['email'] ? sanitize($branch['email']) : 'N/A'; ?></div>
                                    </td>
                                    <td>
                                        <?php if ($branch['manager_id']): ?>
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="avatar-mini d-flex align-items-center justify-content-center rounded-circle" style="width: 28px; height: 28px; background-color: var(--bg-tertiary); color: var(--accent-primary); font-size: 0.75rem; font-weight: bold; border: 1px solid var(--border-color);">
                                                    <?php echo strtoupper(substr($branch['manager_first'], 0, 1) . substr($branch['manager_last'], 0, 1)); ?>
                                                </div>
                                                <div>
                                                    <div class="fw-semibold text-secondary" style="font-size: 0.825rem;">
                                                        <?php echo sanitize($branch['manager_first'] . ' ' . $branch['manager_last']); ?>
                                                    </div>
                                                    <div class="font-mono text-muted" style="font-size: 0.7rem;">
                                                        <?php echo sanitize($branch['manager_code']); ?>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php else: ?>
                                    ...
                                    <td class="pe-4 text-end">
                                        <div class="d-inline-flex gap-2">
                                            <a href="<?php echo base_url('index.php?route=branches-edit&id=' . $branch['id']); ?>" class="btn btn-outline-secondary btn-xs p-1" title="Edit details">
                                                <i class="bi bi-pencil-square" style="font-size: 0.9rem; padding: 2px;"></i>
                                            </a>
                                            <button type="button" class="btn btn-outline-danger btn-xs p-1" title="Soft Delete" onclick="confirmDelete(<?php echo $branch['id']; ?>, '<?php echo sanitize(addslashes($branch['name'])); ?>')">
            ...
            <?php if ($total_pages > 1): ?>
                <div class="d-flex justify-content-between align-items-center p-3 border-top" style="border-color: var(--border-color) !important; background-color: var(--bg-primary);">
                    <div class="text-muted small">
                        Showing page <strong><?php echo $page; ?></strong> of <strong><?php echo $total_pages; ?></strong> (Total: <strong><?php echo $total_rows; ?></strong> branches)
                    </div>
                    <nav>
                        <ul class="pagination pagination-sm mb-0">
                            <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                                <a class="page-link" href="<?php echo base_url('index.php?route=branches&page=' . ($page - 1) . '&search=' . urlencode($search) . '&status=' . urlencode($status_filter)); ?>" style="background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-secondary);">Previous</a>
                            </li>
                            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                    <a class="page-link" href="<?php echo base_url('index.php?route=branches&page=' . $i . '&search=' . urlencode($search) . '&status=' . urlencode($status_filter)); ?>" style="<?php echo $page == $i ? 'background-color: var(--accent-primary); border-color: var(--accent-primary); color: white;' : 'background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-secondary);'; ?>"><?php echo $i; ?></a>
                                </li>
                            <?php endfor; ?>
                            <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
                                <a class="page-link" href="<?php echo base_url('index.php?route=branches&page=' . ($page + 1) . '&search=' . urlencode($search) . '&status=' . urlencode($status_filter)); ?>" style="background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-secondary);">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
        <div class="modal-content" style="background-color: var(--bg-secondary); border: 1px solid var(--border-color);">
            ...
            <div class="modal-body py-3">
                <p class="small text-muted mb-2">Are you sure you want to soft-delete/deactivate this branch?</p>
                <div class="p-2.5 rounded border mb-3 text-center bg-primary-subtle" style="background-color: var(--bg-primary); border-color: var(--border-color) !important;">
                    <strong id="deleteTargetName" style="color: var(--text-primary);">Branch Name</strong>
                </div>
            ...
            <div class="modal-footer border-0 pt-0">
                ...
                <form id="deleteForm" action="<?php echo base_url('index.php?route=branches-delete'); ?>" method="POST">
                    <?php echo csrf_field(); ?>
